...
data.file.name      <- "summary_bmi_latest_mean_prev_cw_adult18_corrected.csv"                           # Set data filename
country.file.name   <- "country-list-2020.csv"
...
source("NCD_RisC_commented_prevalence_bmi_code_FUNCTIONS.R")                            # Loads functions
...
subset   <- subset[order(subset$Superregion,subset$Region,subset$Country,subset$data_year),]
...
colnames(subset) <- c("iso","uid","sex.final","age","number","prev","data_year",
                      "coverage","scope","country","region","sregion", "perurb")                                # Rename columns for analysis
attach(subset)            
